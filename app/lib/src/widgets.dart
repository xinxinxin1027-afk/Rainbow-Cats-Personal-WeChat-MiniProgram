...
class GlassSurface extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    ...
    final Widget interior = ClipRRect(
        ...
        child: DecoratedBox(
          decoration: BoxDecoration(
            gradient: LinearGradient(
              ...
              colors: <Color>[
                Color.alphaBlend(wash.withAlpha(26), Colors.white.withAlpha(214)),
                Color.alphaBlend(wash.withAlpha(18), Colors.white.withAlpha(148)),
              ],
            ),
            borderRadius: borderRadius,
            border: Border.all(color: Colors.white.withAlpha(132), width: 1),
          ),
          child: Material(
...
class OriginalImage extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    final Widget fallback = DecoratedBox(
      ...
      child: const Center(
        child: Icon(Icons.pets_rounded, color: RainbowDesign.accentDeep, size: 42),
      ),
    );
    ...
    if (source != null && source.isNotEmpty) {
      if (source.startsWith('data:image/')) {
        ...
        image = Image.network(source, height: height, width: width, fit: fit,
            errorBuilder: (_, __, ___) => fallback);
      } else if (source.startsWith('file://') || source.startsWith('/')) {
        final String path = source.startsWith('file://') ? source.substring(7) : source;
        image = Image.file(File(path), height: height, width: width, fit: fit,
            errorBuilder: (_, __, ___) => fallback);
      } else {
        image = Image.asset(source, height: height, width: width, fit: fit,
            errorBuilder: (_, __, ___) => fallback);
      }
    }

    return ClipRRect(
      borderRadius: borderRadius ?? BorderRadius.circular(RainbowDesign.radiusMedium),
            ...
            Positioned(
              ...
              child: Semantics(
                ...
                child: GlassIconButton(
                  ...
                  size: 38,
                  iconSize: 18,
                  tint: Colors.white.withAlpha(130),
  ...
  Widget build(BuildContext context) => ClipRRect(
        ...
        child: GestureDetector(
          ...
          onHorizontalDragUpdate: (DragUpdateDetails details) {
            ...
            _controller.value = (_dragStart - details.primaryDelta! / _maxReveal)
                .clamp(0.0, 1.0)
                .toDouble();
...
class RainbowBottomBar extends StatelessWidget {
  const RainbowBottomBar({required this.index, required this.onChanged, super.key});
  final int index;
